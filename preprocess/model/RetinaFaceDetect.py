...
class FaceCropper:
    ...
    def detect_faces(self, path, re_detect=True):

        if re_detect or self.last_box is None:

            face_data = RetinaFace.detect_faces(path, threshold=0.95)

            if not face_data:
                return

            box = np.array(face_data['face_1']['facial_area'])
            score = face_data['face_1']['score']
            ...
            if self.last_box is None:
                ...
                if self.min_shift <= dist <= self.max_shift and box_len > 100 and score > 0.75:
    ...
    def __call__(self, img, re_detect=True):
        # Detect faces

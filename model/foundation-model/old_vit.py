...
from .module import Block, DropPath, Mlp, _cfg, PatchEmbed
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def __init__(self, img_size=128, patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=0, num_frames=30, tubelet_size=1,
                 use_checkpoint=False, checkpoint_num=0, use_learnable_pos_emb=True, clip_return_layer=1,
    ...
    def forward(self, x):
        x, x_clip_vis = self.forward_features(x)
        x = self.head(x)
...
@register_model
def pretrain_umt_base_patch16_128(pretrained=False, **kwargs):
    model = PretrainVisionTransformer(
        img_size=128,
        patch_size=16,
        encoder_embed_dim=768,

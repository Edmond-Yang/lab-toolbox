...
def process_video_threads(rgb_tensor, params, b, t):
    ...
    return b, t, processed_frame.transpose(2, 0, 1)
...
def process_video_batch(rgb_tensor, params):
    ...
    #     future_to_vid = {
    #         executor.submit(process_video_threads, rgb_tensor, params, b, t): (b, t)
    #         for b in range(B) for t in range(T)
    #     }
    #     for future in concurrent.futures.as_completed(future_to_vid):
    #         b, t, frame = future.result()
    #         processed_tensor[b, :, t] = torch.from_numpy(frame).to(rgb_tensor.device)
    #
    # return processed_tensor

    for b in range(B):
        for t in range(T):
            frame = process_video_threads(rgb_tensor, params, b, t)[-1]
            processed_tensor[b, :, t] = torch.from_numpy(frame).to(rgb_tensor.device)
...
def apply_augmentation(rgb):
    ...
    return process_video_batch(rgb, light_params)

...
def augment_speed_clip_only(clip, frames_per_clip, channels, speed_slow, speed_fast):
    ''' 只對傳進來的 clip（短影片）做 speed augment & interpolate '''
    vid_len = len(clip)
    # 隨機選一個起點，確保後面還有足夠的長度
    while True:
        ...
        target_len = int(np.round(frames_per_clip * speed_c))

        print(target_len)
        start_idx  = np.random.randint(0, vid_len - target_len + 1)
    ...
    clip_segment = prepare_clip(clip_segment, channels)
    interped_clip = interpolate_clip(clip_segment, frames_per_clip)
    return interped_clip, speed_c

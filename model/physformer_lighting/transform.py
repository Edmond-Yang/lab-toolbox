...
def _process_a_video(rgb, params, b, t):

    frame = rgb.permute(1, 2, 0).contiguous().cpu().numpy() * 255
    frame = frame.astype(np.uint8)

    bgr_frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)

    if params['add_light'] == True:
        processed_frame = add_parallel_light(
            ...
            mode=params['mode'],
            linear_decay_rate=params['linear_decay_rate'],
            transparency=params['transparency']
        )
    elif params['add_brightness'] == True:
        processed_frame = adjust_brightness(
            bgr_frame,
            brightness_factor=params['brightness_factor']
        )
    else:
        raise ValueError("No augmentation is applied.")

    processed_frame = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB) / 255.0

    return b, t, processed_frame



def process_video_batch(rgb_tensor, params):

    B, C, T, H, W = rgb_tensor.shape
    processed_tensor = rgb_tensor.clone()

    # Use a thread pool to process each frame in parallel
    max_workers = 1
    with concurrent.futures.ThreadPoolExecutor(max_workers) as executor:
        future_to_vid = {
            executor.submit(_process_a_video, rgb_tensor[b, :, t, :, :], params, b, t): (b, t)
            for b in range(B) for t in range(T)
        }
        for future in concurrent.futures.as_completed(future_to_vid):
            b, t, processed_frame = future.result()
            processed_tensor[b, :, t] = torch.from_numpy(
                processed_frame.transpose(2, 0, 1)
            ).to(rgb_tensor.device)
            
    return processed_tensor

def apply_augmentation_light(rgb):
    light_params = {
    ...
    if random.random() > light_params['possibility']:
        augment_rgb = rgb
    else:
        if light_params['is_fix'] == True:
            # fixed light position
            augment_rgb = process_video_batch(rgb, light_params)
        else:
